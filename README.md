...
- **実機に寄せた 3D モデル** — R61P 電源ユニット（黒ヘッダ、端子台6ねじ、上ヒンジの ▼PULL カバー）、R00CPU（READY／ERROR／P RUN 表示）、R35B ベース（DIN レール、スロット、緑基板）を再現。
...
- **three.js r128** による WebGL 3D 描画（オービットカメラはタッチ対応の自作実装）
- ビルド不要・依存パッケージなしの**単一 HTML ファイル**
- モデル・テクスチャ・UI・手順ロジックをすべて 1 ファイル内で完結
